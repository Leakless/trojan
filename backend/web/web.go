...
// all: 前缀确保包含以 _ 或 . 开头的文件(如打包器产出的 _plugin-vue_export-helper-*.js),
// 否则这些静态资源不会被嵌入, 请求会落到需要鉴权的 NoRoute 而返回 401
...
func Start(host string, port, timeout int, isSSL bool) {
	// 容器化/首次启动自举: 确保用户表存在(CREATE TABLE IF NOT EXISTS, 幂等)
	if cfg := core.GetConfig(); cfg != nil && cfg.Mysql.Enabled {
		cfg.Mysql.CreateTable()
	}
	router := gin.Default()
	...
	router.Use(Auth(router, timeout, isSSL).MiddlewareFunc())
	trojanRouter(router)
	userRouter(router)
	dataRouter(router)
	commonRouter(router)
