...
func queryUserList(db *sql.DB, sql string, args ...interface{}) ([]*User, error) {
	var (
		username    string
		encryptPass string
		passShow    string
...
func (mysql *Mysql) CleanDataByName(usernames []string) error {
	...
	runSql := "UPDATE users SET download=0, upload=0 WHERE BINARY username in (" + strings.Join(placeholders, ",") + ")"
	if _, err := db.Exec(runSql, args...); err != nil {
		fmt.Println(err)
		return err
	}
	return nil
...
func (mysql *Mysql) PageList(curPage int, pageSize int) (*PageQuery, error) {
	...
	userList, err := queryUserList(db, "SELECT * FROM users LIMIT ?, ?", offset, pageSize)
	if err != nil {
		fmt.Println(err)
		return nil, err
	}
...
func (mysql *Mysql) GetData(ids ...string) ([]*User, error) {
	...
	if len(ids) > 0 {
		...
		for _, sid := range ids {
			// 强制转为整数, 避免任何非数字内容进入 SQL
			n, err := strconv.Atoi(strings.TrimSpace(sid))
			if err != nil {
				return nil, fmt.Errorf("非法的用户id: %s", sid)
			}
			placeholders = append(placeholders, "?")
			args = append(args, n)
		}
		querySQL = querySQL + " WHERE id in (" + strings.Join(placeholders, ",") + ")"
	}
	userList, err := queryUserList(db, querySQL, args...)
	if err != nil {
		fmt.Println(err)
		return nil, err
	}
